...
function Toolbar({ className, ...props }: React.ComponentProps<typeof BaseToolbar.Root>) {
  return (
    <BaseToolbar.Root
      className={cn(
        'bg-popover outline-border flex items-center gap-2.5 rounded-md p-1 shadow-xs outline',
        className,
      )}
      {...props}
    />
...
function ToolbarSeparator({
  ...
  return (
    <BaseToolbar.Separator className={cn('bg-border h-6 w-px shrink-0', className)} {...props} />
  )
}
